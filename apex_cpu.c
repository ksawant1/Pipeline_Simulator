...
static void
print_stage_NOP(const char *name, const CPU_Stage *stage)
{
    printf("%-15s: pc(%d) ", name, stage->pc);
    printf("NOP");
...
APEX_fetch(APEX_CPU *cpu)
{
    APEX_Instruction *current_ins;
     if(!cpu->fetch.has_insn){
         print_stage_NOP("Fetch", &cpu->fetch);
     }
...
{ 
    if(cpu->decode.rs1!=0 && cpu->execute.rd!=0){
   ...
   if(!cpu->decode.has_insn)
   {
            ...
            print_stage_NOP("Decode/RF", &cpu->decode);
           // print_stage_NOP("Fetch", &cpu->fetch);
...
{   
    if(!cpu->execute.has_insn)
   {
            ...
            print_stage_NOP("Execute", &cpu->execute);
   }
   
   
   
   if(cpu->decode.rs2!=0 && cpu->execute.rd!=0){
   ...
   { 
    ...
    if (cpu->execute.has_insn)
    {
        /* dependency stalling logic */
        /*check if next instructions contain common sources or destinations */
        ...
        switch (cpu->execute.opcode)
        {
            ...
            case OPCODE_CMP:
            {
                ...
                if(cpu->execute.rs1_value == cpu->execute.rs2_value)
                    ...
                    cpu->zero_flag = FALSE;
                printf("compare flga %d",cpu->zero_flag);
            }
        }
...

{
    ...
    if(!cpu->memory.has_insn)
   {
            ...
            print_stage_NOP("Memory", &cpu->memory);
   }
    if (cpu->memory.has_insn)
    {
...
{ 
    if(!cpu->writeback.has_insn)
   {
            ...
            print_stage_NOP("Writeback", &cpu->writeback);
   }
    if (cpu->writeback.has_insn)
    {
